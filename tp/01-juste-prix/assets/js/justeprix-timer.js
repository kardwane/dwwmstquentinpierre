...
// todo : ajout pour timer
//je définit la vmax de me progress coté JS en reprenant la valeur renseigné coté HTML
const maxTimePerRound = progressBar.max;

let timer = 0, // le timer en lui même
	timerInterval; // me permet de stocker mes intervalles de temps

const upTimer = () => {
	// a chaque appel, j'ajoute 1 a mon timer
	timer++;
	//je met à jour son affichage
	document.querySelector("#timer").textContent =
		Math.floor(timer / 10) <= 1
			? Math.floor(timer / 10) + " seconde"
			: Math.floor(timer / 10) + " secondes";
	// je fais progresser ma progressBar en lui mettant a jour sa valeur
	progressBar.value = timer;
	//verification si depassement du temps alloué
	if (timer >= maxTimePerRound) {
		timer = 0;
		tryNumber++;
		//sauf si on a perdu, dans ce cas on ne prépare pas le propchain tour, on stop le jeu
		if (tryNumber >= 7) {
			//Defaite
			endOfGame(false);
			//video facepalm
		...
		//sinon, on prépare le tour suivant
		newTurn();
	}
};
...
// fonction de fermeture du timer
function timerStop(interval) {
...
function endOfGame(res) {
	...
	//je desactive le bouton proposer
...
restartBtn.addEventListener("click", function () {
	...
	// todo : ajout pour timer
	timerStart// je relance le timer
	// todo : fin ajout

	//je réactive le bouton proposer
